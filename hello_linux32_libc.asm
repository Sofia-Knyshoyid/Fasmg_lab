;   Linux 32-bit executable, which uses 
;	only static or no-pie libraries.
;	Needs linking. Using gcc as a linker driver. 
;
; Сompile the source:
;   INCLUDE=x86/include ./fasmg.x64 hello_linux32_libc.asm 
;   gcc hello_linux32_libc.o -o hello_linux32_libc -static -m32
;
; or:
;	INCLUDE=x86/include ./fasmg.x64  hello_linux32_libc.asm 
;   gcc hello_linux32_libc.o -o hello_linux32_libc -no-pie -m32
;
; Stripping the executable could be useful too:
;   strip hello_linux32_libc
;
